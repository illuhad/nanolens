...
template<class T>
class stable_circular_buffer
{
public:
  stable_circular_buffer(std::size_t buffer_size)
  ...
  std::size_t get_used_buffer_size() const
  { return size(); }
  
  std::size_t add_element(const T& x)
  {
    if(_end_index < _buffer_size - 1)
    {
      ++_end_index;
      _buffer[_end_index] = x;
      return _end_index;
    }
    else
    {
      // overwrite elements starting from the beginning
      if(_beg_index == _buffer_size - 1)
        _beg_index = 0;
      else
        ++_beg_index;
      
      _buffer[_beg_index] = x;
      return _beg_index;
    }
  }
